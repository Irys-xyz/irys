...
sed "s/\${RETENTION_DAYS}/$RETENTION_DAYS/g" "$CONFIG_DIR/ilm-policy.json" | \
...
sed "s/\${ES_REPLICAS}/$ES_REPLICAS/g" "$CONFIG_DIR/index-template.json" | \
...
response=$(sed "s/\${ES_REPLICAS}/$ES_REPLICAS/g" "$CONFIG_DIR/index-settings.json" | \
