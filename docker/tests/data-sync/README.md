...
Build the Irys Docker image before running the test:

```bash
./build_image.sh
```

NOTE:  This will build the current version of the Irys repo it is being run in.
...
### Without Test (Development/Debugging)

Start the cluster without running the automated test:
```bash
./start_cluster.sh
```

This starts all 3 Irys nodes and the observability stack, then waits for nodes to be ready. Useful for:
