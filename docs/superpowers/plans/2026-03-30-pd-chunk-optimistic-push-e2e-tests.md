# PD Chunk Optimistic Push E2E Tests — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Three E2E integration tests verifying PD chunk optimistic push across a 3-node network (happy path, cache-hit shortcut, pending fetch reconciliation).

**Architecture:** New test file `pd_chunk_optimistic_push.rs` in `crates/chain-tests/src/programmable_data/`. Shared `PdPushTestContext` and `setup_pd_push_test(fanout)` function modeled on the existing `pd_chunk_p2p_pull.rs` pattern. Test 3 overrides Observer topology so P2P fetch fails, then manually pushes via `gossip_client.push_pd_chunk()`.

**Tech Stack:** Rust, tokio, test_log, IrysNodeTest harness, irys-packing (for chunk unpacking in Test 3)

**Design Doc:** `docs/superpowers/specs/2026-03-30-pd-chunk-optimistic-push-e2e-tests-design.md`

---

### Task 1: Register Test Module

**Files:**
- Modify: `crates/chain-tests/src/programmable_data/mod.rs`
- Create: `crates/chain-tests/src/programmable_data/pd_chunk_optimistic_push.rs`

- [ ] **Step 1: Add module declaration**

In `crates/chain-tests/src/programmable_data/mod.rs`, add the new module after the existing entries:

```rust
pub(crate) mod pd_chunk_optimistic_push;
```

- [ ] **Step 2: Create test file with imports and context struct**
...
Run: `cargo check -p irys-chain-tests 2>&1 | head -20`

Expected: Compilation succeeds (the module is registered but has no tests yet).

- [ ] **Step 4: Commit**

```bash
git add crates/chain-tests/src/programmable_data/mod.rs crates/chain-tests/src/programmable_data/pd_chunk_optimistic_push.rs
git commit -m "test: scaffold pd_chunk_optimistic_push module with PdPushTestContext"
```

---

### Task 2: Implement Setup Function and PD TX Helper

**Files:**
- Modify: `crates/chain-tests/src/programmable_data/pd_chunk_optimistic_push.rs`

- [ ] **Step 1: Add `setup_pd_push_test` function**

Append to `pd_chunk_optimistic_push.rs` after the `PdPushTestContext` struct:
...
- [ ] **Step 2: Add `build_and_inject_real_pd_tx` helper**

Append to `pd_chunk_optimistic_push.rs` after `setup_pd_push_test`:
...
Expected: Compilation succeeds. No tests to run yet.

- [ ] **Step 4: Commit**

```bash
git add crates/chain-tests/src/programmable_data/pd_chunk_optimistic_push.rs
git commit -m "test: add setup_pd_push_test and build_and_inject_real_pd_tx for optimistic push tests"
```

---

### Task 3: Test 1 — Happy Path
...
Append to `pd_chunk_optimistic_push.rs`:
...
async fn heavy3_pd_chunk_optimistic_push_happy_path() -> eyre::Result<()> {
...
Expected: Compiles. Fix any import or type errors.

- [ ] **Step 3: Run the test**

Run: `cargo nextest run -p irys-chain-tests heavy3_pd_chunk_optimistic_push_happy_path --no-capture 2>&1 | tail -30`

Expected: PASS. If the test times out on `wait_for_pd_chunk_in_cache`, check that `pd_optimistic_push_fanout=4` is being set correctly and that Observer is in Genesis's peer list.

- [ ] **Step 4: Commit**

```bash
git add crates/chain-tests/src/programmable_data/pd_chunk_optimistic_push.rs
git commit -m "test: add heavy3_pd_chunk_optimistic_push_happy_path"
```

---

### Task 4: Test 2 — Cache-Hit Shortcut
...
#[test_log::test(tokio::test)]
async fn heavy3_pd_chunk_optimistic_push_cache_hit_shortcut() -> eyre::Result<()> {
    let ctx = setup_pd_push_test(4).await?;
...
Run: `cargo nextest run -p irys-chain-tests heavy3_pd_chunk_optimistic_push_cache_hit_shortcut --no-capture 2>&1 | tail -30`
...
```bash
git add crates/chain-tests/src/programmable_data/pd_chunk_optimistic_push.rs
git commit -m "test: add heavy3_pd_chunk_optimistic_push_cache_hit_shortcut"
```

---

### Task 5: Test 3 — Pending Fetch Reconciliation

**Files:**
- Modify: `crates/chain-tests/src/programmable_data/pd_chunk_optimistic_push.rs`

This test requires additional imports for manual chunk push construction.

- [ ] **Step 1: Add imports for Test 3**

Add these imports at the top of `pd_chunk_optimistic_push.rs` (merge with existing imports):

```rust
use irys_types::gossip::PdChunkPush;
use irys_types::ChunkFormat;
use irys_types::chunk_provider::{ChunkStorageProvider, PdChunkPusher};
use irys_types::PeerAddress;
```

Note: `irys_packing` is already in `chain-tests/Cargo.toml` dependencies. In edition 2024, crate names are automatically available as path roots — no `use irys_packing;` import needed. Just call `irys_packing::unpack(...)` directly.

- [ ] **Step 2: Write the custom setup helper for Test 3**
...
async fn setup_pd_push_test_for_reconciliation() -> eyre::Result<PdPushTestContext> {
    ...
    observer_config.trusted_peers = vec![PeerAddress {
        ...
        gossip: format!(
            "127.0.0.1:{}",
            block_producer.node_ctx.config.node_config.gossip.public_port
        )
        .parse()
        .expect("valid SocketAddr"),
...
#[test_log::test(tokio::test)]
async fn heavy3_pd_chunk_optimistic_push_reconciles_pending_fetch() -> eyre::Result<()> {
    ...
    .await?;
    info!("PD tx injected on Genesis (fanout=0, no auto push): {:?}", tx_hash);
    ...
    // Verify chunk is in Observer's cache.
    assert!(
        ctx.observer
            .node_ctx
            .chunk_data_index
            .get(&(publish_ledger, ctx.data_start_offset))
            .is_some(),
        "Chunk should be in Observer's ChunkDataIndex after push reconciliation",
    );
...
Expected: Compiles. Fix any import or type errors (especially around `ChunkFormat`, `PdChunkPush`, `PdChunkPusher` imports and `irys_packing::unpack` signature).

- [ ] **Step 5: Run the test**

Run: `cargo nextest run -p irys-chain-tests heavy3_pd_chunk_optimistic_push_reconciles_pending_fetch --no-capture 2>&1 | tail -40`

Expected: PASS. If Observer times out on `wait_until_height`:
- Check that the manual push reached Observer (look for tracing logs from `handle_pd_chunk_push` and `handle_optimistic_push`)
- Verify Observer's block index has the offset (it needs this for push verification)
- Check if Observer can reach Genesis's gossip port (even though it's not in trusted_peers)

- [ ] **Step 6: Commit**

```bash
git add crates/chain-tests/src/programmable_data/pd_chunk_optimistic_push.rs
git commit -m "test: add heavy3_pd_chunk_optimistic_push_reconciles_pending_fetch"
```
...
- [ ] **Step 1: Run all 3 tests together**

Run: `cargo nextest run -p irys-chain-tests pd_chunk_optimistic_push --no-capture 2>&1 | tail -20`

Expected: All 3 tests PASS.

- [ ] **Step 2: Run formatting and linting**

Run: `cargo fmt --all && cargo clippy --workspace --tests --all-targets 2>&1 | head -30`

Expected: No warnings or errors. Fix any clippy issues.

- [ ] **Step 3: Run flakiness check (2 iterations)**

Run: `cargo xtask flaky -i 2 -- -E 'test(pd_chunk_optimistic_push)' 2>&1 | tail -20`

Expected: All tests pass both iterations.
...
```bash
git add crates/chain-tests/src/programmable_data/pd_chunk_optimistic_push.rs
git commit -m "test: fix formatting and clippy warnings in optimistic push tests"
```
...
| File | Action |
|------|--------|
| `crates/chain-tests/src/programmable_data/mod.rs` | Add `pub(crate) mod pd_chunk_optimistic_push;` |
| `crates/chain-tests/src/programmable_data/pd_chunk_optimistic_push.rs` | **NEW** — 3 tests + 2 setup functions + context struct + helper |

## Key Reference Files

| File | Lines | What |
|------|-------|------|
| `crates/chain-tests/src/programmable_data/pd_chunk_p2p_pull.rs` | 69-277 | Pattern for `setup_pd_p2p_test` and `build_and_inject_real_pd_tx` |
| `crates/chain-tests/src/utils.rs` | 1343-1397 | `wait_for_pd_chunk_in_cache`, `wait_for_ready_pd_tx` |
| `crates/chain-tests/src/utils.rs` | 682-729 | `testing_peer_with_signer` (trusted_peers setup) |
| `crates/chain-tests/src/utils.rs` | 735-831 | `testing_peer_with_assignments` |
| `crates/chain-tests/src/utils.rs` | 1790, 2520-2539 | `mine_block_without_gossip`, `gossip_block_to_peers` |
| `crates/actors/src/pd_service.rs` | 804-909 | `handle_optimistic_push` (code under test) |
| `crates/actors/src/pd_service/cache.rs` | 152-179 | `insert_unreferenced` |
| `crates/p2p/src/gossip_client.rs` | 2336-2339 | `PdChunkPusher for GossipClient` impl |
| `crates/p2p/src/gossip_data_handler.rs` | 76 | `pub gossip_client: GossipClient` |
| `crates/types/src/gossip.rs` | 466-476 | `PdChunkPush` struct |
| `crates/types/src/chunk.rs` | 158-182 | `PackedChunk` struct |
| `crates/packing/src/lib.rs` | 20-46 | `unpack(&PackedChunk, iterations, chunk_size, chain_id) -> UnpackedChunk` |
