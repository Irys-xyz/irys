...
- **`DbSyncMode`** (`Durable` | `SafeNoSync` | `UtterlyNoSync`) — explicit database sync mode. Wraps `reth_db::mdbx::SyncMode` so it can live in `irys-types` without depending on `reth_db`. A local helper function `db_sync_mode_to_mdbx()` converts between the types in each crate that needs it, because Rust's orphan rule prevents a `From` impl when neither type is local.
