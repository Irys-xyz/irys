...
- `WaitForStepError::Stalled` — `global_step` did not advance for `progress_timeout`. The spawned task no longer panics here; it returns `VdfValidationResult::Stalled { current_step }` and the dispatch (`ValidationCoordinator::handle_stalled_vdf_task`) decides, under two guards:
  - **Canonicality (re-poison safety).** Only a `Canonical`/`CanonicalExtension` block is requeued (re-validated). A `Fork`/`Unknown` block **panics** (`StallPanicReason::NonCanonical`): re-sending its fast-forward steps would re-poison a re-anchored buffer (a partition-recovery re-anchor rebuilds the buffer to the canonical tip and drains the fast-forward queue; fork steps share the same global step numbers and would be accepted verbatim by `store_step`).
  - **Advancement-keyed bound (liveness).** The retry counts only CONSECUTIVE stalls with **no buffer advancement** (`current_step` carried in the variant; `last_stall_step` on the task). Any advance resets it. After `MAX_CONSECUTIVE_NOPROGRESS_STALLS` frozen stalls the classifier **panics** (`StallPanicReason::FrozenWriter`). A genuinely frozen writer therefore still crashes per the never-mislabel rule; a heal cascade (the buffer advancing as predecessors heal) never trips it.

  This refines the earlier unconditional-panic policy. The reason a re-anchor-drain stall is healable but a dead-writer stall is not: a partition-recovery re-anchor can drop an in-flight canonical fast-forward step (the recovery adopts the canonical fork incrementally), stranding a mining-paused follower at the tip — but requeuing the canonical block re-sends that verified-canonical step, which `run_vdf` consumes even while paused (its fast-forward drain runs before the pause gate), so the buffer advances and the wait succeeds. See `design/docs/vdf-partition-recovery-reanchor.md`. **Crucially, the non-cooperative watchdog still skips the `Wait*` stages** (below), so this advancement-keyed bound is the *only* stall backstop for a wait — it must reset on advancement rather than count raw retries, or a deep heal cascade would crash spuriously (and an unbounded requeue would reintroduce the original livelock). On any panic, the propagation is unchanged: `resume_unwind` onto the service task → global panic hook → SIGINT → 45s shutdown watchdog → process abort.
...
Collapsing the two into a single `Err` would re-introduce the original failure mode: dropping a block on cooperative cancel is wrong (it is still potentially valid, just deprioritised). Mapping `Stalled` to `VdfValidationResult::Invalid(_)` would be wrong — surfacing local-infrastructure failure as block-Invalid would fork the node off the network on a programmer-error condition. The original "requeuing a stalled block is wrong … retrying won't help" reasoning held for the dead-writer case this ADR addressed, but **not** for the partition-recovery re-anchor-drain case (above), where a bounded, canonicality-gated requeue re-supplies the dropped canonical step and heals; the advancement-keyed bound preserves the dead-writer crash.
...
- A stalled VDF wait no longer parks validation silently. A canonical block whose fast-forward was dropped by a re-anchor drain is requeued and heals; a non-canonical block, or a buffer frozen past `MAX_CONSECUTIVE_NOPROGRESS_STALLS`, crashes the node (never-mislabel). The block tree learns and acts either way.
