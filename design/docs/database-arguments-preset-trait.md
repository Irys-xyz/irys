...
- Conversion from `DbSyncMode` to `reth_db::mdbx::SyncMode` is centralized via `impl From<DbSyncMode> for reth_db::mdbx::SyncMode` in `crates/types/src/config/node.rs`
