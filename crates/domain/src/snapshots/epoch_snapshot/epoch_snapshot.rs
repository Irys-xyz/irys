...
pub struct EpochSnapshot {
    ...
    /// Refund events generated by applied unpledges at this epoch (if any)
    pub unpledge_refund_events: Option<Vec<UnpledgeRefundEvent>>,
}
...
/// Per-tx refund info produced at epoch for unpledges that retire signer-owned capacity partitions.
#[derive(Debug, Clone)]
...
impl EpochSnapshot {
    ...
    pub fn compute_commitment_state(&mut self, commitments: Vec<CommitmentTransaction>) {
        ...
        let mut refund_events: Vec<UnpledgeRefundEvent> = Vec::new();
        for unpledge in &unpledge_commitments {
            let irys_primitives::CommitmentType::Unpledge {
                partition_hash,
            ...
            // Compute refund amount via LIFO math (count-1) using the tx's count
            let amount = CommitmentTransaction::calculate_pledge_value_at_count(
                &self.config.consensus,
                pledge_count_before_executing
                    .checked_sub(1)
                    .expect("pledge count before executing must always be greater than 1"),
            );

            refund_events.push(UnpledgeRefundEvent {
                tx_id: unpledge.id,
                account: unpledge.signer,
                partition_hash: ph,
                amount,
            });
        ...
        self.unpledge_refund_events = if refund_events.is_empty() {
            None
        } else {
            Some(refund_events)
        };
