...
/// Optional convenience facade for sending requests to the BlockIndex service
#[derive(Debug, Clone)]
pub struct BlockIndexServiceFacade {
    tx: tokio::sync::mpsc::UnboundedSender<BlockIndexServiceMessage>,
}

impl From<&tokio::sync::mpsc::UnboundedSender<BlockIndexServiceMessage>>
    for BlockIndexServiceFacade
{
    fn from(tx: &tokio::sync::mpsc::UnboundedSender<BlockIndexServiceMessage>) -> Self {
        Self { tx: tx.clone() }
    }
}

impl BlockIndexServiceFacade {
    pub fn new(tx: tokio::sync::mpsc::UnboundedSender<BlockIndexServiceMessage>) -> Self {
        Self { tx }
    }

    pub fn sender(&self) -> tokio::sync::mpsc::UnboundedSender<BlockIndexServiceMessage> {
        self.tx.clone()
    }

    pub async fn get_guard(&self) -> eyre::Result<BlockIndexReadGuard> {
        let (resp_tx, resp_rx) = oneshot::channel();
        self.tx
            .send(BlockIndexServiceMessage::GetBlockIndexReadGuard { response: resp_tx })
            .map_err(|_| eyre!("failed to send GetBlockIndexReadGuard"))?;
        resp_rx
            .await
            .map_err(|_| eyre!("GetBlockIndexReadGuard oneshot canceled"))
    }
    ...
    pub async fn migrate_block(
        ...
        self.tx
            ...
            .map_err(|_| eyre!("failed to send MigrateBlock"))?;
        resp_rx
            .await
            .map_err(|_| eyre!("MigrateBlock oneshot canceled"))?
    }

    pub async fn latest_item(&self) -> eyre::Result<Option<BlockIndexItem>> {
        let (resp_tx, resp_rx) = oneshot::channel();
        self.tx
            .send(BlockIndexServiceMessage::GetLatestBlockIndex { response: resp_tx })
            .map_err(|_| eyre!("failed to send GetLatestBlockIndex"))?;
        resp_rx
            .await
            .map_err(|_| eyre!("GetLatestBlockIndex oneshot canceled"))
    }
}
