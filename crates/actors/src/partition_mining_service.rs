...
impl PartitionMiningServiceInner {
    ...
    /// buffer was rewritten onto canonical steps, so the efficient-sampling rotation and
    /// per-step recall cache — both derived from the now-replaced (poisoned) seeds — are
    /// stale. `reset_after_reanchor` discards both and rewinds `last_step_num`, so the
    /// next `get_recall_range` takes the reconstruct path against the corrected buffer
    /// instead of the "next consecutive step" fast path (which would reuse the stale
    /// rotation).
    #[tracing::instrument(level = "trace", skip_all)]
    fn handle_reanchor(&mut self) {
        ...
        self.ranges.reset_after_reanchor();
    }
...
mod tests {
    ...
    /// The consumer half of the in-process VDF re-anchor: `handle_reanchor` — the
    /// one-line dispatch target of `MiningBroadcastEvent::Reanchored` in the service
    /// loop — must rewind the efficient-sampling rotation so the next recall-range
    ...
    fn reanchored_event_rebuilds_recall_ranges_from_corrected_buffer() {
        ...
        inner.handle_reanchor();
        assert_eq!(
            inner.ranges.last_step_num, 0,
            "handle_reanchor must rewind the rotation to force a rebuild"
        );

        // The next query must reconstruct from the corrected buffer: byte-for-byte
        // the pick a fresh rotation fed the corrected steps [1, 3] would make.
