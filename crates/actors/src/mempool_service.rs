...
impl MempoolFacade for MempoolServiceFacadeImpl {
    async fn handle_data_transaction_ingress(
        ...
        self.service
            .send(MempoolServiceMessage::IngressDataTx(tx_header, oneshot_tx))
    ...
    async fn handle_commitment_transaction_ingress(
        ...
        let (oneshot_tx, oneshot_rx) = tokio::sync::oneshot::channel();
        self.service
            .send(MempoolServiceMessage::IngressCommitmentTx(
                commitment_tx,
                oneshot_tx,
            ))
            .map_err(|_| {
    ...
    async fn handle_chunk_ingress(&self, chunk: UnpackedChunk) -> Result<(), ChunkIngressError> {
        let (oneshot_tx, oneshot_rx) = tokio::sync::oneshot::channel();
        self.service
            .send(MempoolServiceMessage::IngressChunk(chunk, oneshot_tx))
            .map_err(|_| {
        ...
        if commitment_status == CommitmentCacheStatus::Accepted {
            ...
            if let Some(pledges_lru) = pop {
                ...
                for pledge_tx in pledges {
                    ...
                    let _ = self
                        .handle_message(MempoolServiceMessage::IngressCommitmentTx(
    ...
    async fn handle_tx_ingress_message(
        ...
        if let Some(chunks_map) = option_chunks_map {
            ...
            for chunk in chunks {
                ...
                let _ = self
                    .handle_message(MempoolServiceMessage::IngressChunk(chunk, oneshot_tx))
                    .await;
    ...
    fn handle_message<'a>(
        ...
        Box::pin(async move {
            match msg {
                ...
                MempoolServiceMessage::IngressChunk(chunk, response) => {
                ...
                MempoolServiceMessage::IngressDataTx(tx, response) => {
