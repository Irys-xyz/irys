...
async fn get_pending_submit_ledger_txs(
    ...
    retain_unconfirmed_submit_candidates(&mut pending_valid_submit_ledger_tx);
...
fn retain_unconfirmed_submit_candidates(
    pending_valid_submit_ledger_tx: &mut HashMap<H256, DataTransactionHeader>,
) {
...
#[cfg(test)]
mod tests {
    use super::retain_unconfirmed_submit_candidates;
    use std::collections::HashMap;

    use irys_types::{
        DataLedger, DataTransactionHeader, DataTransactionHeaderV1,
        DataTransactionHeaderV1WithMetadata, DataTransactionMetadata, H256, IrysAddress,
        IrysSignature, U256,
    };

    fn create_test_data_tx() -> DataTransactionHeader {
    ...
    #[test]
    fn confirmed_and_promoted_submit_candidates_are_excluded() {
        let pending_tx = create_test_data_tx();

        let mut included_tx = create_test_data_tx();
        included_tx.set_metadata(DataTransactionMetadata::with_included_height(7));

        let mut promoted_tx = create_test_data_tx();
        promoted_tx.set_promoted_height(Some(9));

        let mut candidates = HashMap::from([
            (pending_tx.id, pending_tx.clone()),
            (included_tx.id, included_tx),
            (promoted_tx.id, promoted_tx),
        ]);

        retain_unconfirmed_submit_candidates(&mut candidates);

        assert_eq!(candidates.len(), 1);
        assert!(candidates.contains_key(&pending_tx.id));
    }
}
