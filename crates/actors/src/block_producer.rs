...
use reth::{providers::BlockReader, rpc::types::BlockId};
use reth_db::cursor::*;
...
impl Handler<SolutionFoundMessage> for BlockProducerActor {
    ...
    fn handle(&mut self, msg: SolutionFoundMessage, _ctx: &mut Self::Context) -> Self::Result {
        ...
        AtomicResponse::new(Box::pin(
            async move { inner.produce_block(solution).await }
                .into_actor(self)
...
impl BlockProducerInner {
    pub async fn produce_block(
        ...
        let submit_chunks_added =
            calculate_chunks_added(&submit_txs.submit_tx, self.config.consensus.chunk_size);
        let submit_max_chunk_offset = prev_block_header.data_ledgers[DataLedger::Submit]
            .max_chunk_offset
            + submit_chunks_added;
        let submit_txids = submit_txs
            .submit_tx
            .iter()
            .map(|h| h.id)
            .collect::<Vec<H256>>();
        ...
        if is_epoch_block {
            ...
            let entry = self
                .block_tree_guard
                .read()
                .get_commitment_snapshot(&prev.block_hash);
        ...
        self.service_senders
            ...
            .send(EmaServiceMessage::GetPriceDataForNewBlock {
                response: tx,
                height_of_new_block: block_height,
        ...
        let system_txs = system_txs
            .generate_all(commitment_txs_to_bill, &submit_txs.submit_tx)
            .map(|tx_result| {
                let tx = tx_result?;
