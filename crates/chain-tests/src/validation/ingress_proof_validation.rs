...
async fn block_with_unstaked_ingress_proof_signer_rejected() -> eyre::Result<()> {
    ...
    impl BlockProdStrategy for EvilBlockProdStrategy {
        ...
        > {
            // The tx is already confirmed in a previous block's submit ledger,
            // so we only include it in publish_txs (not submit_txs).
    ...
    // 5. Mine a block to confirm tx in submit ledger, triggering ingress proof generation
    genesis_node.mine_block().await?;
    genesis_node
...
#[test_log::test(tokio::test)]
async fn mempool_filters_unstaked_ingress_proofs() -> eyre::Result<()> {
