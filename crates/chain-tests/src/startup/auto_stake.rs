...
#[tokio::test]
async fn heavy_test_auto_stake_pledge(
    #[case] stake: bool,
    #[case] pledges: usize,
    ...
    unsafe {
        std::env::set_var(
            "RUST_LOG",
            "debug,irys_database=off,irys_actors::storage_module_service=off,trie=off,irys_reth::evm=off,engine::root=off,storage::db::mdbx=off,reth_basic_payload_builder=off,providers::db=off,reth_payload_builder::service=off,irys_actors::mining_bus=off,reth_ethereum_payload_builder=off,provider::static_file=off,engine::persistence=off,provider::storage_writer=off,reth_engine_tree::persistence=off,irys_actors::cache_service=off,irys_vdf=off,irys_actors::vdf_service=off,eth_ethereum_payload_builder=off,reth_node_events::node=off,reth::cli=off,reth_engine_tree::tree=off,irys_actors::ema_service=off,irys_efficient_sampling=off,hyper_util::client::legacy::connect::http=off,hyper_util::client::legacy::pool=off,irys_database::migration::v0_to_v1=off,irys_storage::storage_module=off,actix_server::worker=off,irys::packing::update=off,engine::tree=off,irys_actors::mining=error,payload_builder=off,irys_actors::reth_service=off,irys_actors::packing=off,irys_actors::reth_service=off,irys::packing::progress=off,irys_chain::vdf=off,irys_vdf::vdf_state=off,irys_p2p::peer_list=error",
        )
    };
    initialize_tracing();
    ...
    // Use generous timeouts to avoid flakiness under CI load.
    // Typed as u32 for wait_for_mempool_best_txs_shape; cast to usize where needed.
    let seconds_to_wait: u32 = 20;
    ...
    let genesis_node = IrysNodeTest::new_genesis(genesis_config.clone())
        .start_and_wait_for_packing("GENESIS", seconds_to_wait as usize)
        .await;
    ...
    genesis_node
        .wait_for_block_at_height(blk.height, seconds_to_wait as usize)
        .await?;
    ...
    if stake {
        ...
        genesis_node
            .wait_for_mempool_best_txs_shape(0, 0, already_processed_count, seconds_to_wait)
            .await?;
    }
    ...
    if after_epoch {
        ...
        genesis_node
            .wait_for_mempool_best_txs_shape(0, 0, 0, seconds_to_wait)
            .await?;
    }
    ...
    peer_node
        .wait_for_block_at_height(tip_height, seconds_to_wait as usize)
        .await?;
    ...
    peer_node
        .wait_for_block_at_height(head_height, seconds_to_wait as usize)
        .await?;
