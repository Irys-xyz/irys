...
async fn heavy_test_vdf_progress_check_fails_stalled_peer() -> eyre::Result<()> {
    ...
    peer.stop_mining();
    let peer_frozen_step = peer.node_ctx.vdf_steps_guard.read().current_step();
    tracing::info!(
    ...
    let pre_delivery_step = peer.node_ctx.vdf_steps_guard.read().current_step();
    ...
    let post_step = peer.node_ctx.vdf_steps_guard.read().current_step();
    assert_eq!(
