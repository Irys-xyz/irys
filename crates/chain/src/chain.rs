...
impl IrysNode {
    ...
    fn init_block_discovery_service(
        ...
        let block_discovery_inner = BlockDiscoveryServiceInner {
            ...
            db: irys_db.clone(),
            config: config.clone(),
            // Prevalidation keeps its own pool, separate from the one in
            // `ValidationService`. Prevalidation is latency-critical (a block
            // is forwarded only after it passes) while a validation task is a
            // whole VDF step, and rayon has no priority lanes: sharing a pool
            // would park prevalidation behind an in-flight validation batch.
            // The cost is that a node doing both at once runs up to twice
            // `vdf.parallel_verification_thread_limit` threads.
