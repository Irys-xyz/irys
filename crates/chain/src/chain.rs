...
impl IrysNode {
    ...
    pub async fn init(&self) -> eyre::Result<IrysNodeCtx> {
        ...
        let irys_provider = irys_storage::reth_provider::create_provider();
        let actor_main_thread_handle = match (self.data_exists, self.is_genesis) {
                    ...
                    .spawn({
                        ...
                        move || {
                            System::new().block_on(async move {
                                ...
                                let block_index_service_actor = genesis_initialization(
                                    &irys_genesis,
                                    node_config,
                                    &block_index,
                                    &node,
                                ...
                                latest_block_height_tx
                                    .send(irys_genesis.height)
                                    .expect("to be able to send the latest block height");

                                // start the rest of the services
                                let (irys_node, actix_server, vdf_thread, arbiters, reth_node) =
                                    node.init_services(
                                        reth_shutdown_sender,
                    ...
                    .spawn({
                        ...
                        move || {
                            System::new().block_on(async move {
                                ...
                                let block_index = Arc::new(RwLock::new(block_index));
                                let irys_db =
                                    init_irys_db(&node_config).expect("could not open irys db");
    ...
    async fn init_components(&self) -> eyre::Result<IrysNodeCtx> {
        ...
        info!(?base_dir, "Loading existing blockchain state");
        ...
        if PACKING_TYPE != PackingType::CPU && self.storage_config.chunk_size != CHUNK_SIZE {
            error!("GPU packing only supports chunk size {}!", CHUNK_SIZE)
        }
        ...
        let arc_config = Arc::new(self.irys_node_config.clone());
        let difficulty_adjustment_config = DifficultyAdjustmentConfig::new(&self.config);

        // TODO: Hard coding 3 for storage module count isn't great here,
        // eventually we'll want to relate this to the genesis config
        irys_genesis.diff =
            calculate_initial_difficulty(&difficulty_adjustment_config, &self.storage_config, 3)
                .unwrap();
        ...
        let block_index = block_index.init(arc_config.clone()).await.unwrap();
        let latest_block_index = block_index
            .get_latest_item()
            .cloned()
            .expect("the block index must have at least one entry");
        let latest_block_height = block_index.latest_height();
        let block_index: Arc<RwLock<BlockIndex<Initialized>>> = Arc::new(RwLock::new(block_index));
        ...
        .spawn(move || {
            ...
            let reth_node_handle = System::new().block_on(async move {
                ...
                let latest_block = Arc::new(database::block_header_by_hash(&irys_db.tx().unwrap(), &latest_block_index.block_hash)
                    .unwrap()
                    .unwrap());
