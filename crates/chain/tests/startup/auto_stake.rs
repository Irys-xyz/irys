...
async fn test_auto_stake_pledge(#[case] stake: bool, #[case] pledges: usize) -> eyre::Result<()> {
    ...
    // mine blocks
    for num in 0..already_processed_count {
        // wait for the expected txs to show up on the genesis node
        // stake & first pledge aren't anchor chained
        genesis_node
            .wait_for_mempool_best_txs_shape(0, 0, if num == 0 && stake { 2 } else { 1 }, 10)
            .await?;
        // mine an extra block if we're going to mine an epoch block
        // as epoch blocks can't include new commitments
