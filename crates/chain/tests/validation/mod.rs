...
async fn heavy_block_wrong_commitment_order_gets_rejected() -> eyre::Result<()> {
    ...
    let block = Arc::new(SealedBlock::new(header, block.body().clone())?);
...
async fn heavy_block_epoch_commitment_mismatch_gets_rejected() -> eyre::Result<()> {
    ...
    // Send block directly to block tree service for validation
    let err = send_block_to_block_tree(&genesis_node.node_ctx, block.clone(), false)
        .await
...
async fn block_with_invalid_last_epoch_hash_gets_rejected() -> eyre::Result<()> {
    ...
    assert_eq!(
        block_after_epoch.header().height % num_blocks_in_epoch_u64,
        1,
    ...
    let block_after_epoch =
        Arc::new(SealedBlock::new(header, block_after_epoch.body().clone()).unwrap());
