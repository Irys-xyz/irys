...
#[actix_web::test]
async fn heavy_mempool_anchor_test() -> eyre::Result<()> {
    std::env::set_var(
        "RUST_LOG",
        "debug,irys_actors::block_validation=off,storage::db::mdbx=off,reth=off,irys_p2p::server=off,irys_actors::mining=error",
    );
...
// test that a tx with an unknown anchor is a.) accepted, b.) pushed back into the mempool once the anchor item is known
// create two txs, A and B - set B's anchor to A
// post A, it should go into the mempool as anchor pending
// mine a block (to ensure it's not included)
// post B, make sure A is re-submitted
// mine a block, ensure A and B are submitted

// other stuff:
// should get_data_txs return anchor pending?
// should we gossip anchor pending txs?
