...
async fn heavy_two_node_reorg_upto_migration_depth() -> eyre::Result<()> {
    ...
    genesis_config.consensus.get_mut().chunk_size = 32;
    // shorten the epoch so staking + pledging activates quickly
    genesis_config.consensus.get_mut().block_migration_depth = block_migration_depth;
    genesis_config.consensus.get_mut().epoch.num_blocks_in_epoch = num_blocks_in_epoch as u64;
    ...
    // config for node 2
    let mut node2_config = node1.testnet_peer_with_signer(&b_signer);
    node2_config.stake_pledge_drives = true;
    ...
    let node2 = node1
        .testnet_peer_with_assignments_and_name(node2_config, "PEER")
        .await;

    //node1.wait_until_height(0, seconds_to_wait).await?;

    // Mine initial block and wait both nodes to sync
    //let block1 = node1.mine_block().await?;
    node1
    ...
    node2
        ...
        .await?;
    assert_eq!(
        num_blocks_in_epoch as u64,
        node1.get_max_difficulty_block().height
    );
    assert_eq!(
        num_blocks_in_epoch as u64,
        node2.get_max_difficulty_block().height
    );

    // blocks
    let block1 = node1.get_block_by_height(1).await?;

    // Post stake + pledge commitments to node1
    let node2_stake_tx = node2.post_stake_commitment(block1.block_hash).await;
    let node2_pledge_tx = node2.post_pledge_commitment(node2_stake_tx.id).await;
    // Wait for all commitment tx to show up in both node2 mempool
    node1
        .wait_for_mempool(node2_stake_tx.id, seconds_to_wait)
        .await?;
    node2
        .wait_for_mempool(node2_pledge_tx.id, seconds_to_wait)
        .await?;

    let block2 = node1.mine_block().await?;
    node1
        .wait_until_height(num_blocks_in_epoch as u64 + 1, seconds_to_wait)
        .await?;
    node2
        .wait_until_height(num_blocks_in_epoch as u64 + 1, seconds_to_wait)
        .await?;
    assert_eq!(
        num_blocks_in_epoch as u64 + 1,
        node1.get_max_difficulty_block().height
    );
    assert_eq!(
        num_blocks_in_epoch as u64 + 1,
    ...
    let node1_mine_blocks: usize = (block_migration_depth as u64 - block2.height).try_into()?;
    node1.mine_blocks_without_gossip(node1_mine_blocks).await?;
    node1
        .wait_until_height(node1_mine_blocks.try_into()?, seconds_to_wait)
        .await?;

    // confirm heights on both nodes
    assert_eq!(
        block_migration_depth as u64,
        node1.get_max_difficulty_block().height
    );
    assert_eq!(2, node2.get_max_difficulty_block().height);
    // confirm we have not migrate blocks to the index
    assert_eq!(0, node1.get_block_index_height());
    assert_eq!(0, node2.get_block_index_height());

    // Node2 mines one block fewer
    let node2_mine_blocks = (block_migration_depth_minus_one as u64 - block2.height);

    //panic!("gets here");
    //panic!("gets here {:?} ", node2_blocks);
    node2.mine_block().await?;
    //node2.mine_blocks(node2_mine_blocks + 2).await?;
    panic!("never gets here");
    node2
        .wait_until_height(node2_mine_blocks, seconds_to_wait)
        .await?;
    ...
    let tip = node1
        .get_block_by_height(block_migration_depth as u64)
        .await?;
