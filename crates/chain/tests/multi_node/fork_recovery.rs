...
async fn heavy_reorg_tip_moves_across_nodes_publish_txs(
    ...
    let signer_b_genesis_balance: U256 =
        node_a.get_balance(b_signer.address(), genesis_block.evm_block_hash).await;
    let signer_c_genesis_balance: U256 =
        node_a.get_balance(c_signer.address(), genesis_block.evm_block_hash).await;
    ...
    assert_eq!(
        node_a.get_balance(b_signer.address(), a_block1.evm_block_hash).await,
        signer_b_genesis_balance
    );
    assert_eq!(
        node_a.get_balance(c_signer.address(), a_block1.evm_block_hash).await,
        signer_c_genesis_balance
    );
    assert_eq!(
        node_b.get_balance(b_signer.address(), b_block1.evm_block_hash).await,
        signer_b_genesis_balance
    );
    assert_eq!(
        node_b.get_balance(c_signer.address(), b_block1.evm_block_hash).await,
        signer_c_genesis_balance
    );
    assert_eq!(
        node_c.get_balance(b_signer.address(), c_block1.evm_block_hash).await,
        signer_b_genesis_balance
    );
    assert_eq!(
        node_c.get_balance(c_signer.address(), c_block1.evm_block_hash).await,
        signer_c_genesis_balance
    );
    ...
    assert_eq!(
        node_a.get_balance(b_signer.address(), a_block2.evm_block_hash).await,
        signer_b_genesis_balance,
        "Address: {:?}",
        b_signer.address()
    );
    assert_eq!(
        node_a.get_balance(c_signer.address(), a_block2.evm_block_hash).await,
        signer_c_genesis_balance,
        "Address: {:?}",
        c_signer.address()
    );
    ...
    assert_eq!(
        node_b.get_balance(b_signer.address(), b_block2.evm_block_hash).await,
    ...
    assert_eq!(
        node_b.get_balance(c_signer.address(), b_block2.evm_block_hash).await,
        signer_c_genesis_balance,
        "Address: {:?}",
        c_signer.address()
    );
    ...
    assert_eq!(
        node_b.get_balance(b_signer.address(), b_block3.evm_block_hash).await,
    ...
    assert_eq!(
        node_b.get_balance(c_signer.address(), b_block3.evm_block_hash).await,
        signer_c_genesis_balance,
        "Address: {:?}",
        c_signer.address()
    );
    ...
    // Canonical blocks by mining peer: A1, B2, B3, C4
    {
        ...
        assert_eq!(
            node_a.get_balance(b_signer.address(), c_block1.evm_block_hash).await,
            signer_b_genesis_balance,
        );
        assert_eq!(
            node_a.get_balance(c_signer.address(), c_block1.evm_block_hash).await,
            signer_c_genesis_balance,
        );
        ...
        assert_eq!(
            node_a.get_balance(b_signer.address(), c_block4.evm_block_hash).await,
            signer_b_genesis_balance + b_block2.reward_amount + b_block3.reward_amount
                - peer_b_total_fee
                + block_producer_reward
        ...
        assert_eq!(
            node_a.get_balance(c_signer.address(), c_block4.evm_block_hash).await,
