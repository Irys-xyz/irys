...
async fn heavy_staking_pledging_txs_included() -> eyre::Result<()> {
    ...
    let stake_tx = ShadowTransaction::decode_prefixed(&mut block_txs1[1].input().as_ref())
    ...
    let pledge_tx = ShadowTransaction::decode_prefixed(&mut block_txs1[2].input().as_ref())
