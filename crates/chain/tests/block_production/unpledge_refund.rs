...
async fn heavy_unpledge_epoch_refund_flow() -> eyre::Result<()> {
    ...
    tracing::error!(custom.part_hash = ?capacity_pa.partition_hash, "partition hash {:?} unassigned", capacity_pa.partition_hash);
    assert!(
...
> {
    ...
    for (idx, target) in assigned_partitions.iter().enumerate() {
        ...
        .await;
        tracing::debug!(custom.part_hash = ?target.partition_hash(), "Unpledging partition hash {:?}", target.partition_hash());
        signer
            .sign_commitment(&mut unsigned)
