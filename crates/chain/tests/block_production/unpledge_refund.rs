...
async fn heavy_unpledge_epoch_refund_flow() -> eyre::Result<()> {
    ...
    let balance_before_inclusion = genesis_node.get_balance(peer_addr, head_block.evm_block_hash).await;
    ...
    let balance_after_inclusion =
        genesis_node.get_balance(peer_addr, inclusion_block.evm_block_hash).await;
    assert_eq!(
    ...
    let balance_after_epoch = genesis_node.get_balance(peer_addr, last_block.evm_block_hash).await;
...
async fn heavy_unpledge_all_partitions_refund_flow() -> eyre::Result<()> {
    ...
    let balance_before_inclusion =
        genesis_node.get_balance(genesis_signer.address(), head_block.evm_block_hash).await;
    ...
    let balance_after_inclusion =
        genesis_node.get_balance(genesis_signer.address(), inclusion_block.evm_block_hash).await;
    assert_eq!(
        balance_after_inclusion,
    ...
    let balance_after_epoch =
        genesis_node.get_balance(genesis_signer.address(), epoch_block.evm_block_hash).await;
