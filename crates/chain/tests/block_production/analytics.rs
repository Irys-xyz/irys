...
async fn test_blockprod_with_evm_txs() -> eyre::Result<()> {
    ...
    let generate_tx_with_size = |a: &IrysSigner, data_size: usize, price_value: irys_types::U256, price_fee: irys_types::U256| -> eyre::Result<(DataTransaction, Vec<u8>)> {
        let mut data_bytes = vec![0_u8; data_size];
        rand::thread_rng().fill(&mut data_bytes[..]);

        let tx = a.create_publish_transaction(data_bytes.clone(), None, price_value, price_fee)?;
    ...
    for account in [&account1, &account2, &account3] {
        ...
        let tx_with_data = generate_tx_with_size(account, data_size, price_info.value, price_info.fee)?;
    ...
    for i in 0..20 {
        ...
        for (i, a) in accounts.iter().enumerate() {
            ...
            let new_price_info = node
                ...
                .await
                .expect("Failed to get price");
            (*tx, *data_bytes) = generate_tx_with_size(a, new_data_size, new_price_info.value, new_price_info.fee)
