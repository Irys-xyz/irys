...
async fn test_blockprod_with_evm_txs() -> eyre::Result<()> {
    ...
    for account in [&account1, &account2, &account3] {
        ...
        let tx_with_data =
            generate_tx_with_size(account, data_size, price_info.value, price_info.fee)?;
    ...
    for i in 0..20 {
        ...
        for (i, a) in accounts.iter().enumerate() {
            ...
            let new_price_info = node
                ...
                .expect("Failed to get price");
            (*tx, *data_bytes) =
                generate_tx_with_size(a, new_data_size, new_price_info.value, new_price_info.fee)
