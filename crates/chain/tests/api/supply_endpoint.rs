...
/// Mine blocks and wait for migration to database.
...
fn validate_supply_invariants(
    ...
    let total = U256::from_str_radix(&supply.total_supply, 10)?;
    let genesis = U256::from_str_radix(&supply.genesis_supply, 10)?;
    let emitted = U256::from_str_radix(&supply.emitted_supply, 10)?;
    let cap = U256::from_str_radix(&supply.inflation_cap, 10)?;

    eyre::ensure!(
        total == genesis + emitted,
    ...
    let expected_cap = ctx
        ...
        .inflation_cap
        .amount;
    eyre::ensure!(cap == expected_cap, "Inflation cap should match config");

    let inflation_progress: f64 = supply.inflation_progress_percent.parse()?;
    eyre::ensure!(
        inflation_progress >= 0.0 && inflation_progress <= 100.0,
