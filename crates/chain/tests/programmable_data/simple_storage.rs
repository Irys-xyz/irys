...
use crate::programmable_data::basic_test::IrysProgrammableDataBasic::IrysProgrammableDataBasicCalls::get_storage;
...
sol!(
    ...
    #[sol(rpc)]
    IrysSimpleStorage,
    "../../fixtures/contracts/out/IrysSimpleStorage.sol/SimpleStorage.json"
);

#[tokio::test]
async fn test_simple_storage() -> eyre::Result<()> {
    let temp_dir = setup_tracing_and_temp_dir(Some("test_simple_storage"), false);
    ...
    let mut deploy_fut = Box::pin(IrysSimpleStorage::deploy(alloy_provider.clone()));
    ...
    // debug!("storage: {:?}, storage2 {:?}", storage, storage2);
