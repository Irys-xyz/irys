...
impl Ranges {
    ...
    pub fn next_recall_range(&mut self, step: u64, seed: &H256, partition_hash: &H256) -> usize {
        ...
        let range = if self.last_range_pos == 0 {
            ...
            let next_range_pos = (rng.next_pseudorand_num() % self.last_range_pos as u32) as usize; // usize (one word in current CPU architecture) to u32 is safe in 32bits of above architectures
