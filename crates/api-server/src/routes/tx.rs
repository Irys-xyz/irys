...
use irys_database::{
    database, db::IrysDatabaseExt as _, reth_db::transaction::DbTx, tables::IngressProofs,
};
use irys_types::{
...
pub async fn get_tx_is_promoted(
    ...
    match state
        .db
        .view_eyre(|tx| Ok(tx.get::<IngressProofs>(tx_header.data_root)))
    {
        Err(_error) => Err(ApiError::Internal {
            err: String::from("db error"),
        }),
        Ok(proofs) => Ok(web::Json(
            proofs
                .expect("expected proofs result from transaction")
                .is_some(),
        )),
    }
}
