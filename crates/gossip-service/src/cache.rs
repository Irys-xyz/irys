...
pub struct GossipCache {
    ...
    chunks: Arc<RwLock<HashMap<ChunkPathHash, HashMap<IpAddr, Instant>>>>,
    transactions: Arc<RwLock<HashMap<H256, HashMap<IpAddr, Instant>>>>,
    blocks: Arc<RwLock<HashMap<BlockHash, HashMap<IpAddr, Instant>>>>,
}
...
impl GossipCache {
    ...
    pub fn record_seen(&self, peer_ip: IpAddr, data: &GossipData) -> GossipResult<()> {
        let now = Instant::now();
        match data {
            GossipData::Chunk(unpacked_chunk) => {
                let mut chunks = self.chunks.write().map_err(|e| GossipError::Cache(e.to_string()))?;
            ...
            GossipData::Transaction(irys_transaction_header) => {
                let mut txs = self.transactions.write().map_err(|e| GossipError::Cache(e.to_string()))?;
                let peer_map = txs.entry(irys_transaction_header.id).or_default();
                peer_map.insert(peer_ip, now);
            }
            GossipData::Block(combined_block) => {
                let mut blocks = self.blocks.write().map_err(|e| GossipError::Cache(e.to_string()))?;
    ...
    pub fn has_seen(&self, peer_ip: &IpAddr, data: &GossipData, within: Duration) -> GossipResult<bool> {
        let now = Instant::now();
        
        let result = match data {
            GossipData::Chunk(unpacked_chunk) => {
                let chunk_path_hash = unpacked_chunk.chunk_path_hash();
                let chunks = self.chunks.read().map_err(|e| GossipError::Cache(e.to_string()))?;
                chunks
                    .get(&chunk_path_hash)
            ...
            GossipData::Transaction(transaction) => {
                let txs = self.transactions.read().map_err(|e| GossipError::Cache(e.to_string()))?;
                txs
                    .get(&transaction.id)
                    .and_then(|peer_map| peer_map.get(peer_ip))
                    .map(|&last_seen| now.duration_since(last_seen) <= within)
            ...
            GossipData::Block(block) => {
                let blocks = self.blocks.read().map_err(|e| GossipError::Cache(e.to_string()))?;
                blocks
    ...
    pub fn cleanup(&self, older_than: Duration) -> GossipResult<()> {
        ...
        let cleanup_chunks = |map: &mut HashMap<H256, HashMap<IpAddr, Instant>>| {
        ...
        {
            let mut chunks_guard = self.chunks.write().map_err(|e| GossipError::Cache(e.to_string()))?;
            let mut chunks = chunks_guard.deref_mut();
            cleanup_chunks(&mut chunks);
        }

        {
            let mut txs_guard = self.transactions.write().map_err(|e| GossipError::Cache(e.to_string()))?;
            let mut txs = txs_guard.deref_mut();
            cleanup_chunks(&mut txs);
        }

        {
            let mut blocks_guard = self.blocks.write().map_err(|e| GossipError::Cache(e.to_string()))?;
