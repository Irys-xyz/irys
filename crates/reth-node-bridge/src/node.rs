...
pub type RethNodeExitHandle2 = NodeHandle<RethNode, RethNodeAddOns>;
...
pub async fn run_node4(
    ...
    let handle = builder
        .with_types_and_provider::<EthereumNode, reth_provider::providers::BlockchainProvider2<
            NodeTypesWithDBAdapter<EthereumNode, RethDbWrapper>,
        >>()
        .with_components(
            EthereumNode::components(), // .executor(IrysExecutorBuilder {
                                        //     precompile_state_provider: PrecompileStateProvider {
                                        //         provider: irys_provider.clone(),
                                        //     },
                                        // })
...
pub async fn run_node5(
    ...
    let handle = builder
        ...
        .launch_with_fn(|builder| {
            ...
            //     latest_block,
            // );
            let launcher = reth_node_builder::DefaultNodeLauncher::new(
                builder.task_executor().clone(),
                builder.config().datadir(),
            );
            builder.launch_with(launcher)
